...
//            Xoa khong thanh cong
    ...
    function uploadFile($file){
        $target_dir = $_SERVER['DOCUMENT_ROOT']."/php/public/uploads/";
...
//            Xoa khong thanh cong
    ...
    private function loadViewCustomer()
    {
        $method = filter_input(INPUT_GET, 'method', FILTER_SANITIZE_SPECIAL_CHARS);
        if (empty($method)){
            $data = $this->db->readAll('user');
            require 'admin/list-user.php';
        }else{
            $id = $_GET['id'];
            $this->db->delete('user',$id);
            header('Location: '.home_base_url().'admin/index.php?page=customer');
            die;
        }
